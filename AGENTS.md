...
- 工具内页头部图标、`all-tools.html`、hub 页都必须与这里保持一致，不要再各自维护另一套图标。
- 主图标统一优先使用 emoji，不使用占位问号、几何符号、ASCII 文本、HTML 实体作为长期方案。
- 同类工具优先保持语义一致，不要同类页面每个都换一套图标风格。
...
1. 先看同类工具是否已有稳定图标语义，优先复用。
2. 优先表达“用户任务”，不是表达底层实现。
3. 如果改动 `icon`，要检查 `all-tools`、相关 hub 页和工具内页头部是否仍然协调。
